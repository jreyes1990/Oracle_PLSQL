...
/*  PARAMETRO 1:
    ...
    STG_TRANSACCION LEFT JOIN Todas las dimensiones por medio de los COD. Ambiente DW. 
    Lleno tabla de hechos (todos los key llenos) y no hechos (key nulos). 
    
    PARAMETRO 2:
    Procedimiento de HECHOS (PRC_CONSTRUYE_HECHOS), Parámetro 2. 
    Basado en Tabla NO_HECHOS LEFT JOIN DIMENSIONES. Ambiente DW.
*/
...
IS
    ...
    CURSOR CR_CONSTRUYE_NO_HECHOS(
        ...
        LEFT JOIN DIM_MEDIOS_TRANSPORTE H ON(A.CODIGO_TRANSPORTE=H.CODIGO_MEDIOS_TRANSPORTE)
        LEFT JOIN DIM_ALMACENADORA I ON(A.CODIGO_ALMACENADORA=I.CODIGO_ALMACENADORA)
        WHERE (CASE WHEN (B.AGENTE_KEY IS NOT NULL AND C.REGIMEN_KEY IS NOT NULL AND D.ADUANA_KEY IS NOT NULL AND F.PAIS_KEY IS NOT NULL AND FF.PAIS_KEY IS NOT NULL AND G.CLIENTE_KEY IS NOT NULL AND H.MEDIOS_TRANSPORTE_KEY IS NOT NULL AND I.ALMACENADORA_KEY IS NOT NULL) THEN 1 ELSE 0 END) = P_VALIDA
        ;
    --
    ...
    BEGIN
        IF P_OPCION IS NOT NULL THEN
            ...
            ELSIF P_OPCION = 2 THEN
                FOR n IN CR_CONSTRUYE_NO_HECHOS(1) LOOP
                    BEGIN
                        IF n.VALIDA_KEYS IN(0, 1) THEN
                            IF n.VALIDA_KEYS = 1 THEN
                                -- INSERTANDO A LA TABLA DE HECHOS
                                BEGIN
                                    INSERT INTO HECHOS 
                                           (AGENTE_KEY, REGIMEN_KEY, ADUANA_KEY, OPERACION_KEY, PAIS_ORIGEN_KEY, PAIS_VENDEDOR_KEY, CLIENTE_KEY, 
                                            CODIGO_TRANSPORTE_KEY, ALMACENADORA_KEY, FECHA_VALIDACION_KEY, FECHA_ADICION_KEY, FECHA_PRESENTACION_KEY, 
                                            PESO_TOTAL, VALOR_DOLARES, VALOR_QUETZALES, VALOR_FLETES, VALOR_SEGUROS, VALOR_OTROS_GASTOS) 
                                    VALUES(n.AGENTE_KEY, n.REGIMEN_KEY, n.ADUANA_KEY, n.OPERACION_KEY, n.PAIS_ORIGEN_KEY, n.PAIS_VENDEDOR_KEY, n.CLIENTE_KEY,
                                           n.CODIGO_TRANSPORTE_KEY, n.ALMACENADORA_KEY, n.FECHA_VALIDACION_KEY, n.FECHA_ADICION_KEY, n.FECHA_PRESENTACION_KEY,
                                           n.PESO_TOTAL, n.VALOR_DOLARES, n.VALOR_QUETZALES, n.VALOR_FLETES, n.VALOR_SEGUROS, n.VALOR_OTROS_GASTOS);
                                EXCEPTION WHEN OTHERS THEN
                                    RAISE_APPLICATION_ERROR(-20001, 'PRC_CONSTRUYE_HECHOS => Error al insertar los datos a la tabla de HECHOS: ');
                                END;
                            END IF;
                    ...
                    EXCEPTION
                        WHEN VALIDACION_NO_DEFINIDO THEN
                            RAISE_APPLICATION_ERROR(-20001, 'PRC_CONSTRUYE_HECHOS => La Validación ['||n.VALIDA_KEYS||'], No esta definido dentro del cursor CR_CONSTRUYE_NO_HECHOS');
                    END;
                END LOOP;
            ELSE
